...
+ partial derivative of the SSE cost function for a particular weight can be calculated as

 $$\Delta w_j = - \eta \frac{\partial J}{\partial w_j} = - \eta \sum_{i} (t^{(i)} - o^{(i)})(- x^{(i)}_{j}) = \eta \sum_i (t^{(i)} - o^{(i)})x^{(i)}_{j}$$

+ we can apply a simultaneous weight update similar to the perceptron rule:
...
<figure style="width: 650px" class="align-center">
  <img src="/images/deep-learning/perceptron_learning_rate.png" alt="Perceptron Learning Rate">
  <figcaption></figcaption>
</figure>
...
where $$\mu_j$$ is the sample mean of the feature $$x_{j}$$ and $$\sigma_j$$ the standard deviation
{: style="text-align: center"}


## Training MLP

> In neural networks, you forward propagate to get the output and compare it with the real value to get the error. To minimize the error, you propagate backwards by finding the derivative of error with respect to each weight and then subtracting this value from the weight value.<br/>
...
<figure style="width: 500px" class="align-center">
  <img src="/images/deep-learning/mlp-training.png" alt="MLP Training">
  <figcaption></figcaption>
</figure>
...
1. At $$L_2$$, hidden layer :<br/>
 $$\begin{align}
 \qquad \qquad \qquad \qquad a_{1}^{(2)} &= f(W_{11}^{(1)}x_1 + W_{12}^{(1)} x_2 + W_{13}^{(1)} x_3 + b_1^{(1)})  \\
 a_2^{(2)} &= f(W_{21}^{(1)}x_1 + W_{22}^{(1)} x_2 + W_{23}^{(1)} x_3 + b_2^{(1)})  \\
 a_3^{(2)} &= f(W_{31}^{(1)}x_1 + W_{32}^{(1)} x_2 + W_{33}^{(1)} x_3 + b_3^{(1)})
 \end{align}$${: style="text-align: center"}
<br/>*bias units don’t have inputs or connections going into them, since they always output the value +1.*
2. At $$L_3$$, output neuron :<br/>
 $$\begin{align}
 \qquad \qquad \qquad \qquad h_{W,b}(x) &= a_1^{(3)} \\
 h_{W,b}(x) &=  f(W_{11}^{(2)}a_1^{(2)} + W_{12}^{(2)} a_2^{(2)} + W_{13}^{(2)} a_3^{(2)} + b_1^{(2)})
 \end{align}$$
 <br/>*neural network defines a hypothesis $$\operatorname{h_{W,b}}(x)$$, for fixed setting of the parameters $$(W,b)$$, that outputs a real number.*
3. let $$z_{i}^{(l)}$$ denote the total weighted sum of inputs to unit $$i$$ in layer $$l$$, including the bias term :<br/>
 $$\begin{align}
 \qquad \qquad \qquad \qquad z_{i}^{2} & = \sum_{j=1}^{n}(W_{ij}^{1}x_j + b_{i}^{1}) \\
 a^{(l)}_i &= f(z^{(l)}_i)
 \end{align}$$
4. extending the activation function $$\operatorname{f}(⋅)$$ to apply to vectors in an element-wise fashion :<br/>
 $$\begin{align}
 \qquad \qquad \qquad \qquad z^{(2)} &= W^{(1)} x + b^{(1)} \\
 a^{(2)} &= f(z^{(2)}) \\
 z^{(3)} &= W^{(2)} a^{(2)} + b^{(2)} \\
 h_{W,b}(x) &= a^{(3)} = f(z^{(3)})
 \end{align}$$
 <br/>*by organizing our parameters in matrices and using matrix-vector operations, we can take advantage of fast linear algebra routines to quickly perform calculations in our network*
...
2. For each output unit $$i$$ in layer $$n_l$$ (the output layer), set<br/>
 $$\begin{align}
 \qquad \qquad \qquad \qquad \delta^{(n_l)}_i &= \frac{\partial}{\partial z^{(n_l)}_i} \;\; \frac{1}{2} \left\|y - h_{W,b}(x)\right\|^2 \\
 &= - (y_i - a^{(n_l)}_i) \cdot f'(z^{(n_l)}_i)
 \end{align}$$

3. each node $$i$$ in layer $$l$$ (where $$l = n_l-1, n_l-2, n_l-3, \ldots$$), set<br/>
 $$\begin{align}
 \qquad \qquad \qquad \qquad \delta^{(l)}_i = \left( \sum_{j=1}^{s_{l+1}} W^{(l)}_{ji} \delta^{(l+1)}_j \right) f'(z^{(l)}_i)
 \end{align}$$

4. Compute the desired partial derivatives, which are given as:
 $$\begin{align}
 \qquad \qquad \qquad \qquad \frac{\partial}{\partial W_{ij}^{(l)}} J(W,b; x, y) &= a^{(l)}_j \delta_i^{(l+1)} \\
...
3. Update parameters,<br/>
 $$\begin{align}
 \qquad \qquad \qquad \qquad W^{(l)} &= W^{(l)} - \alpha \left[ \left(\frac{1}{m} \Delta W^{(l)} \right) + \lambda W^{(l)}\right] \\
 b^{(l)} &= b^{(l)} - \alpha \left[\frac{1}{m} \Delta b^{(l)}\right]
 \end{align}$$
 <br/>*where $$\alpha$$ is the learning rate parameter and $$\lambda$$ is called the weight decay parameter.*
